...
class Attention_SIGFormer(nn.Module):
    def __init__(self, sample_hop, model_arc, device="cuda:0"):
    ...
    def model_to_cuda(self):
        self.spec_lambda = self.spec_lambda.to(self.device) # 谱域注意力系数
        self.path_emb = self.path_emb.to(self.device) # 路径编号对应的嵌入
...
class CFT(dns.DNS):
    def __init__(self, user_num, item_num, sse, spe,
        ...
        for _ in range(params.layer_num):
            layer = Attention_SIGFormer(params.walk_len,
                                  params.model_arc, params.device)
    ...
    def model_to_cuda(self):
        super().model_to_cuda()
        self.sse = self.sse.to(self.params.device)
        self.spe = self.spe.to(self.params.device)
        for i in range(self.params.layer_num):
